...
def delete_by_bill_ids(base_url: str, key: str, table: str, bill_ids: list[str]) -> None:
    if not bill_ids:
        return
...
def main() -> None:
    ...
    # ── 기존 데이터 삭제 (재실행 idempotent) ──────────────────────────────────
    for table in (
        "bill_cost_triggers",
        "non_attachment_reason_classifications",
        "cost_estimate_structures",   # cascade → items → variables/amounts
    ):
    ...
    for a in amounts_all:
        ...
        amt_rows.append({
            ...
            "amount_thousand":  a.get("amount_thousand"),
